...
mp.events.add('initPhone', () => {
    phone = mp.browsers.new('package://cef/resources/views/menus/phone.html');
});
// // //
...
mp.events.add('phoneCallback', (itemid, event, data) => {
    global.anyEvents.SendServer(() => mp.events.callRemote('Phone', 'callback', itemid, event, data));
    //mp.gui.chat.push(itemid+":"+event+":"+data);
});
mp.events.add('phoneNavigation', (btn) => {
    global.anyEvents.SendServer(() => mp.events.callRemote('Phone', 'navigation', btn));
});
// // //
