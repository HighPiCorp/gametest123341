...
mp.keys.bind(Keys.VK_I, false, function () {
    ...
    if (!global.loggedin || global.phoneOpen == 1 || chatActive || editing || cuffed || localplayer.getVariable('InDeath') == true) return;
...
mp.events.add('inventoryContext', (act, type, element) => {
    ...
    switch(act){
		case 0:
            global.anyEvents.SendServer(() => mp.events.callRemote('inventoryContext', type, element, 'use'));
            break;
		case 1:
            global.anyEvents.SendServer(() => mp.events.callRemote('inventoryContext', type, element, 'transfer'));
            break;
		case 2:
            global.anyEvents.SendServer(() => mp.events.callRemote('inventoryContext', type, element, 'drop'));
            break;
	}
})
