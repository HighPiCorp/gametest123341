...
namespace NeptuneEvo.Core
{
    ...
    public class VehicleStreaming : Script
    {
        ...
        public void Event_EntityDeleted(Entity entity)
        {
            ...
            catch (Exception e) { Log.Write("Event_EntityDeleted: " + e.StackTrace); }
        }
        ...
        public static void SetDoorState(Vehicle veh, DoorID door, DoorState state)
        {
            ...
            if (door == DoorID.DoorTrunk)
            {
                veh.SetSharedData("trunkStatus", state == DoorState.DoorOpen ? true : false);
            }
            else if (door == DoorID.DoorHood)
            {
                veh.SetSharedData("hoodStatus", state == DoorState.DoorOpen ? true : false);
            }
            else
            {
                UpdateVehicleSyncData(veh, data);
                NAPI.ClientEvent.TriggerClientEventInDimension(veh.Dimension, "VehStream_SetVehicleDoorStatus_Single", veh.Handle, (int)door, (int)state);
            }
        }
        ...
        public void VehicleFixStreamIn(Player player, Vehicle veh)
        {
            try
            {
                if (veh != null && NAPI.Entity.DoesEntityExist(veh))
                {
                    ...
                    {
                        var id = veh.GetData<int>("ID");
                        vData.Add(VehicleManager.Vehicles[id].Components);
                        vData.Add(VehicleManager.Vehicles[id].Dirt);
                    }
                    Trigger.ClientEvent(player, "VehStream_FixStreamIn", veh.Handle, JsonConvert.SerializeObject(vData));
                }
                return;
            }
            catch (Exception e) { Log.Write("VehStream_RequestFixStreamIn: " + e.StackTrace, nLog.Type.Error); return; }
        }
        ...
        public void SetVehicleDirtLevel(Player player, Vehicle vehicle, float dirt)
        {
            ...
            catch (Exception e) { Log.Write("VehStream_SetVehicleDirt: " + e.StackTrace, nLog.Type.Error); }
        }
        ...
        public static void SetVehicleDirt(Vehicle veh, float dirt)
        {
            ...
            if (veh.HasData("ACCESS") && veh.GetData<string>("ACCESS") == "PERSONAL")
            {
                ...
                VehicleManager.Vehicles[id].Dirt = dirt;
            }
        ...
        public static bool UpdateVehicleSyncData(Vehicle veh, VehicleSyncData data)
        {
            ...
            catch (Exception e) { Log.Write("UpdateVehicleSyncData: " + e.StackTrace, nLog.Type.Error); return false; }
        }
    }
}
