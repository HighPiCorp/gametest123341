...
const vehiclesHaveAutopilot = ["dilettante", "surge", "imorgon", "khamelion", "neon", "raiden", "cyclone", "tezeract", "voltic", "aetron", "ocnetrongt", "taycan", "modelx", "tesla", "teslas", "avtr", "ocnlamtmc"];

mp.keys.bind(0x73, true, () => { // F4
  if (localPlayer.vehicle) {
    const findedVehicle = vehiclesHaveAutopilot.find((vehicle) => localPlayer.vehicle.isModel(mp.game.joaat(vehicle)));
    ...
    if (localPlayer.vehicle.getPedInSeat(-1) == localPlayer.handle && autoPilotActivated == false) {
      ...
      {
        const blipID 	= mp.game.invoke('0x186E5D252FA50E7D'); // _GET_BLIP_INFO_ID_ITERATOR
        const firstBlip = mp.game.invoke('0x1BEDE233E6CD2A1F', blipID); // GET_FIRST_BLIP_INFO_ID
        const nextBlip = mp.game.invoke('0x14F96AA50D6FBEA7', blipID); // GET_NEXT_BLIP_INFO_ID
        ...
        for (let i = firstBlip; mp.game.invoke('0xA6DB27D19ECBB7DA', i) != 0; i = nextBlip) // DOES_BLIP_EXIST
        {
          if (mp.game.invoke('0xBE9B0959FFD0779B', i) == 4) // GET_BLIP_INFO_ID_TYPE
          {
            ...
            if (coord) {
              localPlayer.taskVehicleDriveToCoordLongrange(localPlayer.vehicle.handle, coord.x, coord.y, 0.0, 33.0, 2883621, 40.0);
