...
mp.events.add("changePage", (page) => {
	global.anyEvents.SendServer(() => mp.events.callRemote("changePage", page));
});

mp.events.add("farmerBuy", (item, value) => {
	global.anyEvents.SendServer(() => mp.events.callRemote("buyFarmerItem", item, value));
});
mp.events.add("farmerSell", (item, value) => {
	global.anyEvents.SendServer(() => mp.events.callRemote("sellFarmerItem", item, value));
});
