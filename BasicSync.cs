...
namespace NeptuneEvo.Core
{
    class BasicSync : Script
    {
        ...
        //private static List<NetHandle> attachedObjects = new List<NetHandle>();

        public static void AddAttachment(Player player, string attachmentName, bool remove)
        {
            AttachmentSync.AddAttachment(player, attachmentName, remove);
        }
        ...
        public static void AttachObjectToPlayerVehicle(Player player, uint model, string bone, Vector3 posOffset, Vector3 rotOffset)
        {
            var attObj = new AttachedObject(model, bone, posOffset, rotOffset);
            player.SetSharedData("attachedObject", JsonConvert.SerializeObject(attObj));
            Trigger.ClientEventInRange(player.Position, 550, "attachObjectToVehicle", player);
        }
        ...
        public static void FingerPointerStart(Player player)
        {
            ...
            {
                Log.Write("FingerPointerStart.Event: " + e.StackTrace + "\n playerIdsStr: ", nLog.Type.Error);
            }
        }
        ...
        public static void FingerPointerStop(Player player)
        {
            ...
            {
                Log.Write("FingerPointerStop.Event: " + e.StackTrace + "\n playerIdsStr: ", nLog.Type.Error);
            }
        }
        ...
        public static void FingerPointerUpdateData(Player player, string playersIdsStr, float camPitch, float camHeading, bool fingerIsBlocked, bool fingerIsFirstPerson)
        {
            ...
            {
                Log.Write("FingerPointerUpdateData.Event: " + e.StackTrace, nLog.Type.Error);
            }
        }
        ...
        public static void SetInvisible(Player player, bool toggle)
        {
            ...
            catch (Exception e) { Log.Write("InvisibleEvent: " + e.StackTrace, nLog.Type.Error); }
        }
        ...
        internal class AttachedObject
        {
            ...
            public AttachedObject(uint model, string bone, Vector3 pos, Vector3 rot)
            {
                Model = model;
                BoneName = bone;
                PosOffset = pos;
