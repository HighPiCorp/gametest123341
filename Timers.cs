...
namespace NeptuneEvo.SDK
{
    public static class Timers
    {
        ...
        /// <summary>
        /// Проверяет есть ли Таймер с указанным идентификатором
        /// </summary>
        /// <param name="id">Уникальный идентификатор таймера</param>
        ...
        public static string Start(int interval, Action action)
        {
            ...
            {
                Log.Write($"Timer.Start.{id}.Error: {e.StackTrace}", nLog.Type.Error);
                return null;
            }
        }
        /// <summary>
        ...
        {
            if (id is null) {
                throw new Exception("Trying to stop timer with NULL ID");
            }
            ...
            if (timers.ContainsKey(id))
            {
                ...
                Log.Write($"[Timer STOPED] id: {id}", nLog.Type.Success);
                return true;
            }
