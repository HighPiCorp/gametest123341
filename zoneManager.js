...
mp.zones.registerZone = (Vectors, height, zoneName, type, dimension) => {
  ...
  else if(type == mp.zones.types["NPointZone"])
  {
    if(Vectors.length > 2)
    {
      ...
      // Делаем общую нижнюю грань зоны
      let zoneMinPoint = 0;
      Vectors.forEach((zone) => {
        if (zoneMinPoint == 0) zoneMinPoint = zone.z;
        else {
          if (zoneMinPoint > zone.z) zoneMinPoint = zone.z;
        }
      });

      // Опускаем на -1 юнит зону под землю
      Vectors.forEach((zone) => {
        zone.z = zoneMinPoint - 1;
      });
...
mp.zones.drawZone = (Vectors, height, r=255, g=0, b=0, a=255) => {
  //Check if vectors length are more than 2
