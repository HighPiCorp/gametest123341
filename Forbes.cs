    ...
    class Forbes : Script
    {
        ...
        public static int GetPlayerAllMoney(string Name, int add)
        {
            try
            {
                ...
                foreach (int number in VehicleManager.getAllPlayerVehicles(Name))
                {
            ...
            catch (Exception e) { Log.Write("GetPlayerAllMoney: " + e.ToString(), nLog.Type.Error); return 0; }
        }
        ...
        public static List<Dictionary<string, object>> GetSereliazeMajors()
        {
        ...
        [Command("forb",Hide = true)]
        private static void OpenForbesCommand(Player player)
        {
            if (!Group.CanUseCmd(player, "forb")) return; //10lvl
            if (!player.HasMyData("Phone"))
            {
                client.Phone.Phone.OpenPhone(player);

                Dictionary<string, object> dict = new Dictionary<string, object>();
                dict.Add("activeApp", "ForbesApp");
                player.TriggerEvent("CLIENT::PHONE:OPEN_PHONE", JsonConvert.SerializeObject(dict));
