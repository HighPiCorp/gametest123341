...
namespace NeptuneEvo.Core
{
    public class SafeMain : Script
    {
        ...
        public static void CreateSafe(int i, Vector3 position, float rotation, int minamount, int maxamount, string address)
        {
            ...
            cmd.CommandText = "INSERT INTO `safes` SET " +
              "`minamount`=@minamount," +
              "`maxamount`=@maxamount," +
              "`pos`=@pos," +
              "`address`=@address," +
            ...
            cmd.Parameters.AddWithValue("@pos", string_pos);
        ...
        public static void RemoveSafe(int ID)
        {
            ...
            MySqlCommand cmd = new MySqlCommand();
            cmd.CommandText = "DELETE FROM `safes` WHERE `id`=@id";
        ...
        {
            if (!player.HasMyData("HEIST_DRILL"))
            {
            ...
            canBeClosed = false;
            Manager.sendFractionMessage((int)Fractions.Manager.FractionTypesEnum.Cityhall, "Кто-то пытается взломать дверь в хранилище мэрии.");
            Manager.sendFractionMessage((int)Fractions.Manager.FractionTypesEnum.LSPD, "Кто-то пытается взломать дверь в хранилище мэрии.");
            Manager.sendFractionMessage((int)Fractions.Manager.FractionTypesEnum.FIB, "Кто-то пытается взломать дверь в хранилище мэрии.");
            Manager.sendFractionMessage((int)Fractions.Manager.FractionTypesEnum.Army, "Кто-то пытается взломать дверь в хранилище мэрии.");
            Manager.sendFractionMessage((int)Fractions.Manager.FractionTypesEnum.Sheriff, "Кто-то пытается взломать дверь в хранилище мэрии.");
        }
        ...
        public static void lockCrack(Player player, string name)
        {
            NAPI.Task.Run(() =>
            {
                try
                {
                    ...
                    Doormanager.SetDoorLocked(player.GetMyData<int>("DOOR"), false, 0);
                    //player.FreezePosition = false;
                    Trigger.ClientEvent(player, "hideLoader");
                    //Main.StopT(player.GetMyData("LOCK_TIMER"), "timer_20");
                    player.ResetMyData("LOCK_TIMER");
        ...
        public void onPlayerEnterVehicle(Player player, Vehicle vehicle, sbyte seatid)
        {
            try
            {
                if ((player.HasMyData("HAND_MONEY") || player.HasMyData("HEIST_DRILL")) && player.VehicleSeat == 0 && vehicle.Class != 8)
                {
        ...
        public void narkosale_onEntityEnterColShape(ColShape shape, Player player)
        {
            try
            {
                //entity.SetMyData("INTERACTIONCHECK", 14);
                player.SetMyData("INTERACTIONCHECK", 14);
            }
            catch (Exception e) { Log.Write("narkosale_onEntityEnterColShape: " + e.ToString(), nLog.Type.Error); }
        }

        public void narkosale_onEntityExitColShape(ColShape shape, Player player)
        {
            try
            {
        ...
        public static void openSafe(Player player, params object[] arguments)
        {
            try
            {
                if (!player.HasMyData("temp_SafeID")) return;

                Safe safe = Safes.FirstOrDefault(s => s.ID == player.GetMyData<int>("temp_SafeID"));
                if (safe == null) return;

                if (!player.HasMyData("CURRENT_STAGE"))
                {
                ...
                {
                    int stage = player.GetMyData<int>("CURRENT_STAGE");
                    if (stage == 2)
                    {
                    ...
                    {
                        stage++;
                        player.SetMyData("CURRENT_STAGE", stage);
            ...
            catch (Exception e) { Log.Write("dialPressed: " + e.StackTrace, nLog.Type.Error); }
        }
        ...
        public static void interactSafe(Player player)
        {
            ...
            else
            {
                ...
                if (Fractions.Manager.FractionTypes[Main.Players[player].Fraction.FractionID] != 1)
                {
                ...
                foreach (var p in nearestPlayers)
                {
                    ...
                    if (Fractions.Manager.FractionTypes[Main.Players[p].Fraction.FractionID] == 1) gangsters++;
                }

                if (gangsters == 0 && Main.Players[player].AdminLVL < 1)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"С Вами должен быть как минимум ещё один человек", 3000);
                    //return;
                }

                safe.Occupier = player;
                player.SetMyData("CURRENT_STAGE", 0);
                ...
                Manager.sendFractionMessage((int)Fractions.Manager.FractionTypesEnum.LSPD, $"Сейф по адресу {safe.Address} пытаются взломать");
                Manager.sendFractionMessage((int)Fractions.Manager.FractionTypesEnum.FIB, $"Сейф по адресу {safe.Address} пытаются взломать");
                Manager.sendFractionMessage((int)Fractions.Manager.FractionTypesEnum.Sheriff, $"Сейф по адресу {safe.Address} пытаются взломать");
                ...
                if (DateTime.Now >= safe.BlipSet)
                {
                    ...
                    foreach (var p in Main.Players.Keys.ToList())
                    {
                        ...
                        if (Main.Players[p].Fraction.FractionID != (int)Fractions.Manager.FractionTypesEnum.LSPD && Main.Players[p].Fraction.FractionID != (int)Fractions.Manager.FractionTypesEnum.FIB && Main.Players[p].Fraction.FractionID != (int)Fractions.Manager.FractionTypesEnum.Sheriff) continue;
        ...
        {
            if (Main.Players[player].InsideHouseID != -1 || Main.Players[player].InsideGarageID != -1 || Main.Players[player].InsideFamilyGarageID != -1)
            {
        ...
        {
            if (!player.HasMyData("HAND_MONEY"))
            {
        ...
        public static void SafeCracker_Disconnect(Player player, DisconnectionType type, string reason)
        {
            try
            {
                if (player.HasMyData("HAND_MONEY")) dropMoneyBag(player);
                if (player.HasMyData("HEIST_DRILL")) dropDrillBag(player);
        ...
        public static void CMD_RemoveSafe(Player player)
        {
            ...
            if (!player.HasMyData("temp_SafeID"))
            {
                player.SendChatMessage("~r~[Ошибка] ~w~Вы должны быть возле сейфа.");
                return;
            }

            RemoveSafe(player.GetMyData<int>("temp_SafeID"));
        }
        #endregion
        ...
        //                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Вы купили стяжки", 3000);
        //                return;
        //            case 5:
        //                if (Main.Players[player].Money < 600)
        //                {
        ...
        //                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Вы купили мешок", 3000);
        //                return;
        //            case 6:
        ...
        //        }
        //    }
        //    catch (Exception e) { Log.Write("mavrbuy: " + e.StackTrace, nLog.Type.Error); }
        //}
        ...
        private static void callback_safedoor(Player player, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            switch (item.ID)
            {
                case "change":
                    ...
                    if (Main.Players[player].Fraction.FractionID == (int)Fractions.Manager.FractionTypesEnum.Cityhall && Main.Players[player].Fraction.FractionRankID >= 14)
                    {
                        if (isCracking)
                        {
    ...
    public class Safe
    {
        ...
        public void Create()
        {
            ...
            colShape.OnEntityEnterColShape += (shape, player) =>
            {
                try
                {
                    player.SetMyData("temp_SafeID", ID);
                    player.SetMyData("INTERACTIONCHECK", 43);
                }
                catch (Exception e) { Console.WriteLine("colShape.OnEntityEnterColShape: " + e.StackTrace); }
            };
            ...
            colShape.OnEntityExitColShape += (shape, player) =>
            {
                try
                {
                    ...
                    player.SetMyData("INTERACTIONCHECK", 0);
                    Trigger.ClientEvent(player, "dial", "close");
                    player.ResetMyData("temp_SafeID");
                }
                catch (Exception e) { Console.WriteLine("colShape.OnEntityExitColShape: " + e.StackTrace); }
            };
        }
