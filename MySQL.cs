...
namespace NeptuneEvo.SDK
{
    public static class MySQL
    {
        ...
        private static nLog Log = new nLog("MySQL", true);
        ...
        public static void Init()
        {
            ...
            Connection =
                ...
                $"Database={config.TryGet<string>("DataBase", "kfc2")};" +
        ...
        public static void Query(MySqlCommand command, bool debug = false)
        {
            try
            {
                if (Debug || debug) {
                    ...
                    foreach(var param in command.Parameters) paramsString += JsonConvert.SerializeObject(param)+"\n";
            ...
            catch (Exception e) { Log.Debug("Query to DB:\n" + command.CommandText + " Parameters:\n" + command.Parameters); Log.Write(e.ToString(), nLog.Type.Error); }
        }
        /// <summary>
        /// Выполнить запрос без ответа
        /// </summary>
        /// <param name="command">Передаем команду в виде строки</param>
        public static void Query(string command, bool debug = false)
        {
        ...
        /// </summary>
        /// <param name="command">Передаем заранее составленную команду</param>
        public static async Task QueryAsync(MySqlCommand command, bool debug = false)
        {
            try
            {
                if (Debug || debug) {
        ...
        /// </summary>
        /// <param name="command">Передаем команду в виде строки</param>
        public static async Task QueryAsync(string command, bool debug = false)
        {
            try
            {
                if (Debug || debug) Log.Debug("Query to DB:\n" + command);
                using (MySqlConnection connection = new MySqlConnection(Connection))
                {
                    await connection.OpenAsync();
            ...
            catch (Exception e) { Log.Debug("Query to DB:\n" + command); Log.Write(e.ToString(), nLog.Type.Error); }
        }
        /// <summary>
        ...
        public static DataTable QueryRead(MySqlCommand command, bool debug = false)
        {
            if (Debug || debug) {
                string paramsString = "";
        ...
        /// <param name="command">Передаем команду в виде строки</param>
        /// <returns>Ответ базы данных в формате таблицы</returns>
        public static DataTable QueryRead(string command, bool debug = false)
        {
            using(MySqlCommand cmd = new MySqlCommand(command))
            {
                return QueryRead(cmd, debug);
            }
        }
        /// <summary>
        ...
        /// <param name="command">Передаем заранее составленную команду</param>
        /// <returns>Ответ базы данных в формате таблицы</returns>
        public static async Task<DataTable> QueryReadAsync(MySqlCommand command, bool debug = false)
        {
            if (Debug || debug) {
                string paramsString = "";
        ...
        /// <param name="command">Передаем заранее составленную команду</param>
        /// <returns>Ответ базы данных в формате таблицы</returns>
        public static async Task<DataTable> QueryReadAsync(string command, bool debug = false)
        {
            using(MySqlCommand cmd = new MySqlCommand(command))
            {
                return await QueryReadAsync(cmd, debug);
            }
        }
