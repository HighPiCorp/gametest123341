...
// const getMinimapAnchor = function () {
//     let sfX = 1.0 / 20.0;
//     let sfY = 1.0 / 20.0;
...
mp.events.add('render', () => {
    if (!global.loggedin || chatActive || editing || cuffed || localplayer.getVariable('InDeath') == true) return;

    mp.game.controls.disableControlAction(0, 48, true);
    if (mp.game.controls.isDisabledControlJustPressed(0, 48)) {

        var minimap = global.init.getMinimapAnchor();
        ...
        if (bigmap.status === 0) {
            ...
            // big
            mp.gui.execute(`HUD.minimapFix=${(minimap.rightX * 100) * 2}`);
