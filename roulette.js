...
mp.game1.streaming.requestAnimDict("anim_casino_b@amb@casino@games@shared@dealer@");
mp.game1.streaming.requestAnimDict("anim_casino_b@amb@casino@games@shared@player@");
mp.game1.streaming.requestAnimDict("anim_casino_b@amb@casino@games@roulette@table");
mp.game1.streaming.requestAnimDict("anim_casino_b@amb@casino@games@roulette@dealer");
mp.game1.streaming.requestAnimDict("anim_casino_b@amb@casino@games@roulette@ped_male@seat_1@regular@01a@base");
mp.game1.streaming.requestAnimDict("anim_casino_a@amb@casino@games@lucky7wheel@male");
mp.game1.streaming.requestAnimDict("anim_casino_a@amb@casino@games@lucky7wheel@female");

mp.game1.streaming.requestIpl("vw_casino_main");
...
// mp.events.add('luckyWheel', (playerId) => {
//   try {
//     // const player = mp.players.atRemoteId(playerId);
//
//     logger.debug("SERVER::CASINO:startRoll start");
//     global.anyEvents.SendServer(() => mp.events.callRemote('SERVER::CASINO:startRoll'));
//
//     // const dict = mp.game.joaat("mp_m_freemode_01") == localplayer.model ?
//     //   'anim_casino_a@amb@casino@games@lucky7wheel@male' : 'anim_casino_a@amb@casino@games@lucky7wheel@female';
//     //
//     // mp.utils.requestAnimDict(dict, () => {
//     //   let wheelPos = new mp.Vector3(1110.2651, 228.62857, -50.7558);
//     //
//     //   localplayer.taskGoStraightToCoord(wheelPos.x, wheelPos.y, wheelPos.z, 1.0, -1, 312.2, 0.0);
//     //
//     //   setTimeout(() => {
//     //     // localplayer.setRotation(0.0, 0.0, 2.464141, 1, true);
//     //     // localplayer.taskPlayAnim(dict, "enter_right_to_baseidle", 8.0, -8.0, -1, 0, 0, false, false, false);
//     //     // let ready_baseidle = false;
//     //     //
//     //     // while (!ready_baseidle) {
...
//     //     // }
//     //     //
//     //     // localplayer.taskPlayAnim(dict, "enter_to_armraisedidle", 8.0, -8.0, -1, 0, 0, false, false, false);
//     //     // let ready_armraisedidle = false;
//     //     //
//     //     // while (!ready_armraisedidle) {
//     //     //   if (!mp.game.invoke('0x1F0B79228E461EC9', player, dict, 'enter_to_armraisedidle')) {
//     //     //     ready_armraisedidle = true;
//     //     //   }
//     //     //   mp.game.wait(0);
//     //     // }
//     //
...
//     //     global.anyEvents.SendServer(() => mp.events.callRemote('SERVER::CASINO:startRoll'));
//     //
//     //     //localplayer.taskPlayAnim(dict, "armraisedidle_to_spinningidle_high", 8.0, -8.0, -1, 0, 0, false, false, false);
//     //   }, 2000);
//     // });
//   } catch (e) {
//     logger.error(e);
...
setTimeout(function () {
	try {
		for (let tbs = 0; tbs < RouletteTablesPos.length; tbs++) {
			RouletteTables[tbs] = {};
			RouletteTables[tbs].table = mp.objects.new(mp.game1.joaat('vw_prop_casino_roulette_01'), new mp.Vector3(RouletteTablesPos[tbs].x, RouletteTablesPos[tbs].y, RouletteTablesPos[tbs].z), {
...
setTimeout(function () {
	try {
		...
		CasinoPeds.forEach(ped => {
			...
			//mp.game1.invoke("0xEA47FE3719165B94", CasinoPedsID[0].handle, "anim_casino_b@amb@casino@games@shared@dealer@", "idle", 1000.0, -2.0, -1, 2, 1148846080, false, false, false)
		});
		n = 0;
	} catch (e) {
		logger.error(e);
...
mp.events.add('delWheelCam', () => {
	try {
		...
		rouletteCamera = null;
		mp.game1.cam.renderScriptCams(false, true, 1000, true, false);
...
// DLC_VW_ROULETTE_BALL_LOOP dlc_vw_table_games_sounds

mp.events.add('spin_wheel', function(tb, needSpins, endTable, endBall, numBall){
	try {
		...
		RouletteTables[tb].ballNum = numBall;

		CasinoPedsID[tb].taskPlayAnim("anim_casino_b@amb@casino@games@roulette@dealer", "spin_wheel", 8.0, 1, -1, 2, 0.0, false, false, false);

		setTimeout(() => {
			mp.game.audio.playSoundFromCoord(-1, "DLC_VW_ROULETTE_BALL_LOOP", ballPos.x, ballPos.y, ballPos.z, "dlc_vw_table_games_sounds", true, 0, false);
		}, 3000);
	...
	try {
		if (setBet && rouletteCamera != null && rouletteTable != null && startRoullete == false && !mp.gui.cursor.visible) {
			if (canDoBets && betObject == null) {
				betObject = mp.objects.new(mp.game1.joaat("vw_prop_chip_100dollar_x1"), new mp.Vector3(RouletteTablesPos[rouletteTable].x, RouletteTablesPos[rouletteTable].y, RouletteTablesPos[rouletteTable].z + 0.4));
				betObject.setCollision(false, false);
			}
			if (betObject != null && canDoBets) {
				if (mp.game1.controls.isDisabledControlJustReleased(0, 25) && !mp.gui.cursor.visible) // ПКМ
				{
					if (closestChipSpot != null) global.anyEvents.SendServer(() => mp.events.callRemote("server_remove_roulette_bet", closestChipSpot));
				}

				if (mp.game1.controls.isDisabledControlJustReleased(0, 24) && !mp.gui.cursor.visible) // ЛКМ
				{
					if (closestChipSpot != null) global.anyEvents.SendServer(() => mp.events.callRemote("server_make_roulette_bet", closestChipSpot));
				}
				...
				if (drawObj != null) {
					...
					//drawObj.position.z = mp.game1.gameplay.getGroundZFor3dCoord(drawObj.position.x, drawObj.position.y, drawObj.position.z, parseFloat(0), false);
			...
			let rightAxisX = mp.game1.controls.getDisabledControlNormal(0, 220);
			let rightAxisY = mp.game1.controls.getDisabledControlNormal(0, 221);
		...
		if (startRoullete == true && rouletteTable != null && !roulleteCam) {
			...
			mp.game1.cam.renderScriptCams(true, true, 2500, true, false);
			roulleteCam = true;
		}
		if (startRoullete == false && rouletteTable != null && roulleteCam) {
			...
			rouletteCamera.setActive(true);
			mp.game1.cam.renderScriptCams(true, true, 1500, true, false);
		}
	} catch (e) {
		logger.error(e);
	}
		...
		for (var i = 0; i < RouletteTables.length; i++) {
			...
			if (RouletteTables[i].table.isPlayingAnim("anim_casino_b@amb@casino@games@roulette@table", "loop_wheel", 3)) {
				...
				if (RouletteTables[i].spins == RouletteTables[i].needSpins && RouletteTables[i].table.getAnimCurrentTime("anim_casino_b@amb@casino@games@roulette@table", "loop_wheel") > 0.99) {
					...
					mp.game.audio.playSoundFromCoord(-1, `dlc_vw_roulette_exit_${RouletteTables[i].ballNum}`, ballPos.x, ballPos.y, ballPos.z, "dlc_vw_table_games_roulette_exit_sounds", true, 0, false);
				}
			}
		}
	} catch (e) {
...
mp.events.add('casinoBet', (val) => {
	try {
		...
		global.anyEvents.SendServer(() => mp.events.callRemote('serverSetRouletteBet', val));
	} catch (e) {
		logger.error(e);
	}
});
...
mp.events.add('bet_roulette', function(table, spot){
	try { //player.taskPlayAnim("anim_casino_b@amb@casino@games@blackjack@player", "place_bet_small", 3.0, 1.0, -1, 2, 0, false, false, false);
		let tablePos = RouletteTablesPos[table];
		let betOffset = tableChipsOffsets[spot];
		let newCardPos = mp.game1.object.getObjectOffsetFromCoords(tablePos.x, tablePos.y, tablePos.z, RouletteTablesHeading[table], betOffset[0], betOffset[1], betOffset[2]);

		betRoulette[table].push(mp.objects.new(mp.game1.joaat(`vw_prop_chip_100dollar_x1`), new mp.Vector3(newCardPos.x, newCardPos.y, newCardPos.z),
			{
...
function getCameraHitCoord()
{
	try {
		...
		// mp.game1.graphics.drawLine(position.x, position.y, position.z, farAway.x, farAway.y, farAway.z, 255, 0, 0, 255);
...
function getClosestChipSpot(vector)
{
	try {
		...
		for (var i = 0; i < tableChipsOffsets.length; i++) {
			...
			let newCordPos = mp.game1.object.getObjectOffsetFromCoords(RouletteTablesPos[rouletteTable].x, RouletteTablesPos[rouletteTable].y, RouletteTablesPos[rouletteTable].z, RouletteTablesHeading[rouletteTable], tableChipsOffsets[i][0], tableChipsOffsets[i][1], tableChipsOffsets[i][2]);
			dist = mp.game1.gameplay.getDistanceBetweenCoords(vector.x, vector.y, vector.z, newCordPos.x, newCordPos.y, newCordPos.z, false);
		...
		if (spot != closestChipSpot) {
			...
			if (spot != null) {
				var key = null;
				var obj = null;
				let newBetPos = mp.game1.object.getObjectOffsetFromCoords(RouletteTablesPos[rouletteTable].x, RouletteTablesPos[rouletteTable].y, RouletteTablesPos[rouletteTable].z, RouletteTablesHeading[rouletteTable], tableChipsOffsets[spot][0], tableChipsOffsets[spot][1], tableChipsOffsets[spot][2]);
				betCoords = newBetPos;

				for (var i = 0; i < tableChipsOffsets[spot][3].length; i++) {
					key = tableChipsOffsets[spot][3][i];
					if (key == "00" || key == "0") {
						let newCardPos = mp.game1.object.getObjectOffsetFromCoords(RouletteTablesPos[rouletteTable].x, RouletteTablesPos[rouletteTable].y, RouletteTablesPos[rouletteTable].z, RouletteTablesHeading[rouletteTable], tableMarkersOffsets[key][0], tableMarkersOffsets[key][1], tableMarkersOffsets[key][2]);
...
mp.keys.bind(0x09, false, function () { // change bet
    ...
    global.anyEvents.SendServer(() => mp.events.callRemote('serverChangeRouletteBet'));
