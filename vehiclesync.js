        ...
        if (veh && mp.vehicles.exists(veh)) {
            if (veh !== undefined) {
                if (state === 0) {
                    ...
                    veh.setDoorOpen(door, false, true);
                }
                else {
...
mp.events.add("VehStream_FixStreamIn", async (entity, data) => {
    //mp.console.logInfo("VehStream_FixStreamIn entity.type: "+entity.type + " mp.vehicles.exists(entity): "+mp.vehicles.exists(entity));
        ...
        if (typeor !== 'undefined') {
            ...
            for (let x = 0; x < 8; x++) {
                if (actualData.Door[x] === 1)
                    entity.setDoorOpen(x, false, true);
...
mp.events.add("entityStreamIn", (entity) => {
    try {
        if (entity.type !== "vehicle") return;
        //mp.console.logInfo("entityStreamIn entity.type: "+entity.type + " mp.vehicles.exists(entity): "+mp.vehicles.exists(entity));
        if (entity && mp.vehicles.exists(entity))
        {

            if (entity.hasVariable("FIXGROUND")) {
              setTimeout(() => {
                entity.setOnGroundProperly();
                if (entity.hasVariable("FIXGROUNDCASINO")) entity.position.z += 0.22;
              }, 2000);
            }
            ...
            if(entity.hasVariable('FREEZE')){
                setTimeout(() => {
                    try {
                        entity.setOnGroundProperly();
                        entity.getVariable('FREEZE') ? entity.freezePosition(true) : entity.freezePosition(false);
                    }
                    catch(e){}
                }, 3000);
            }

            if(entity.hasVariable('trunkPlayer')){
                let bony = entity.getBoneIndexByName("boot");

                var trunkPlayer = entity.getVariable('trunkPlayer');

                if (mp.players.local.isStreamed(trunkPlayer)){
                    trunkPlayer.attachTo(entity.handle, bony, 0, 0, 0.5, 0, 0, 0, true, false, false, false, 0, false);
                }
            }
            ...
            if(entity.getVariable('BoatAnchor') !== undefined)
            {
                mp.game1.invoke('0x75DBEC174AEEAD10', entity, entity.getVariable('BoatAnchor'));
                mp.game1.invoke('0xE3EBAAE484798530', entity, entity.getVariable('BoatAnchor'));
            }

            if (typeor !== 'undefined') {
                actualData = JSON.parse(actualData);
                //mp.console.logInfo("actualData: "+JSON.stringify(actualData));
                ...
                if(entity.hasVariable('trunkStatus')){
                    entity.getVariable('trunkStatus') ? entity.setDoorOpen(5, false, true) : entity.setDoorShut(5, false);
                }

                if(entity.hasVariable('hoodStatus')){
                    entity.getVariable('hoodStatus') ? entity.setDoorOpen(4, false, true) : entity.setDoorShut(5, false);
                }

                if (typeof entity.getVariable('hlcolor') !== 'undefined') {
                    let headlightsColor = entity.getVariable('hlcolor');
                    if (headlightsColor > 0) {
                        entity.setMod(22, 0);
                        entity.toggleMod(22, true);
                        mp.game.invoke('0xE41033B25D003A07', entity.handle, headlightsColor);
                    }
                    else {
                        entity.setMod(22, -1);
                    }
                ...
                if (typeof entity.getVariable('NeonColor') !== 'undefined') {
                    ...
                    //mp.console.logInfo(`NeonColor: ${neonColors[0]} ${neonColors[1]} ${neonColors[2]}`);
                    if (typeof neonColors !== 'object') {
                      neonColors = JSON.parse(neonColors);
                    }

                    entity.setNeonLightsColour(neonColors.Red, neonColors.Green, neonColors.Blue);
                    ...
                    //mp.console.logInfo(`NeonColor: ${neonColors.Red} ${neonColors.Green} ${neonColors.Blue}`);
                }

                if (typeof entity.getVariable('VehicleColors') !== 'undefined') {
                  let vehicleColors = entity.getVariable('VehicleColors');

                  if (typeof vehicleColors !== 'object') {
                    vehicleColors = JSON.parse(vehicleColors);
                  }

                  setTimeout(() => {
                    entity.setCustomPrimaryColour(vehicleColors.PrimColor.Red, vehicleColors.PrimColor.Green, vehicleColors.PrimColor.Blue);
                    entity.setCustomSecondaryColour(vehicleColors.SecColor.Red, vehicleColors.SecColor.Green, vehicleColors.SecColor.Blue);

                    entity.setModColor1(vehicleColors.PrimModColor, 0, vehicleColors.PearlColor);
                    entity.setModColor2(vehicleColors.SecModColor, 0);

                    entity.setExtraColours(vehicleColors.PearlColor, vehicleColors.WheelsColor);
                  }, 1000);
                }
...
mp.events.add("VehStream_SetVehicleAnchor", (entity, toggle) => {
    try {
        if (entity && mp.vehicles.exists(entity)) {
            if (entity !== undefined)
            {
                mp.game1.invoke('0x75DBEC174AEEAD10', entity.handle, toggle);
                mp.game1.invoke('0xE3EBAAE484798530', entity.handle, toggle);
            }
        }
    } catch (e) {
        logger.error(e);
...
mp.events.addDataHandler("trunkStatus", function (entity, data) {
    if (entity && mp.vehicles.exists(entity)) {
        //mp.console.logInfo(`${data}`);
        data ? entity.setDoorOpen(5, false, false) : entity.setDoorShut(5, false);
    }
});

mp.events.addDataHandler("hoodStatus", function (entity, data) {
    if (entity && mp.vehicles.exists(entity)) {
        data ? entity.setDoorOpen(4, false, false) : entity.setDoorShut(5, false);
    }
});
