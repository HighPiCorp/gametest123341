...
mp.keys.bind(global.Keys.VK_5, false, function () {
    ...
    global.showGamertags = !global.showGamertags;
});
                        ...
                        if (tagLabelPool[_player.remoteId] === undefined || reupdateTagLabel[_player.remoteId] === undefined || new Date().getTime() - reupdateTagLabel[_player.remoteId] > 500) {
                            ...
                            if (_player.getVariable('IS_MASK') == true)
                            {
                                if (isAdmin === true) {
                                    ...
                                    text = 'Неизвестный' + ' (' + _player.remoteId + ')';
                                }
                            } else {
                                if (isAdmin === true || mp.storage.data.friends.indexOf(_player.name) != -1 || global.passports[_player.name] != undefined) {
                            ...
                            if (playerFraction > 0) {
                                if (isAdmin === true) {
                                    text = '['+ playerFraction +']'+'[' + playerFractionRankName + '] ' + _player.name + ' (' + _player.remoteId + passportText + ')';
                                }
                                else if(localFraction == playerFraction)
                                {
                                    text = _player.name + ' (' + _player.remoteId + passportText + ')';
                                }
                            }
                            //family
                            if (global.localplayer.getVariable('IS_FAMILY') && _player.getVariable('familycid') != "null" && global.localplayer.getVariable('familycid') === _player.getVariable('familycid'))
                                text = `${_player.name} (${_player.remoteId }${passportText}) `;
