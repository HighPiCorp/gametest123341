...
namespace NeptuneEvo.Core
{
    class VehChangeNumber : Script
    {
        ...
        public static void OnResourceStart()
        {
            try
            {
                ...
                shape.OnEntityEnterColShape += (s, player) =>
                {
                    try
                    {
                        //ent.SetMyData("INTERACTIONCHECK", 800);
                        player.SetMyData("INTERACTIONCHECK", 800);
                    }
                    catch (Exception ex) { Console.WriteLine("shape.OnEntityEnterColShape: " + ex.StackTrace); }
                };
                shape.OnEntityExitColShape += (s, player) =>
                {
                    try
                    {
                        //ent.SetMyData("INTERACTIONCHECK", 0);
                        player.SetMyData("INTERACTIONCHECK", 0);
                    }
                    catch (Exception ex) { Console.WriteLine("shape.OnEntityExitColShape: " + ex.StackTrace); }
                };
        ...
        public static void OpenVehChangeNumberMenu(Player player)
        {
            ...
            Trigger.ClientEvent(player, "popup::open", "CAR_CHANGE_NUMBER", $"Вы действительно хотите изменить гос. номер на авто {veh.DisplayName} ({player.Vehicle.NumberPlate}) за ${Price}?");
        }
        public static void ChangeVehNumber(Player player)
        {
            ...
            catch (Exception e) { RLog.Write("Change Vehicle Number: " + e.StackTrace, nLog.Type.Error); }
        }*/
    }
}