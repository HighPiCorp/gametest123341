...
mp.events.add('startEditing', function (model) {
    var hashModel = mp.game.joaat(model);

    var pos = new mp.Vector3(mp.players.local.position.x+1, mp.players.local.position.y+1, mp.players.local.position.z-0.5);
    object = mp.objects.new(hashModel, pos, //mp.players.local.getCoords(true),
    {
    ...
    //mp.console.logInfo("model: "+model+" -> "+hashModel);
});
...
mp.keys.bind(0x59, false, function () { // Y key
    ...
    object = null;
    global.editing = false;
    //mp.console.logInfo("Editing END");
});
