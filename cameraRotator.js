class CameraRotator {
  ...
  changeLength(dX, dY){
    ...
    this.offsetLength += dY * this.zUpMultipler * -1;

    if(this.offsetLength < this.lBound[0])
    {
      this.offsetLength = this.lBound[0];
    }
    else if(this.offsetLength > this.lBound[1])
    {
      this.offsetLength = this.lBound[1];
    }
  ...
  changePosition() {
    const position = mp.game.object.getObjectOffsetFromCoords(this.basePosition.x, this.basePosition.y,
      this.basePosition.z + this.zUp, this.heading, this.offsetVector.x, this.offsetVector.y + this.offsetLength, this.offsetVector.z);
...
mp.events.add("render", () => {
  ...
  if (mp.game.controls.isDisabledControlPressed(2, 238)) {
    cameraRotator.changeLength(dX, dY);
  }
});
...
function drawDebugText() {
  ...
  message += `\noffsetLength: ${cameraRotator.offsetLength.toFixed(2)}`;
  message += `\nlBound: ${cameraRotator.lBound[0].toFixed(2)} ${cameraRotator.lBound[1].toFixed(2)}`;
  message += `\nzBound: ${cameraRotator.zBound[0].toFixed(2)} ${cameraRotator.zBound[1].toFixed(2)}`;
  message += `\nxBound: ${cameraRotator.xBound[0].toFixed(2)} ${cameraRotator.xBound[1].toFixed(2)}`;
