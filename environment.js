...
let setTimeCMDused = false;

const nowTime = { Hour: 0, Minute: 0 };
const nowDate = { Day: 7, Month: 4, Year: 2018 };
let nowWeather = 'EXTRASUNNY';
...
mp.events.add('Enviroment_Time', (data) => {
  ...
  var res = {time: `${data[0]}:${data[1]}`};

  if(global.oscar2Menu != null && global.oscar2Menu != undefined)
    global.oscar2Menu.call('CEF::phone:update', JSON.stringify(res));

  const time = `${global.init.formatIntZero(nowTime.Hour, -2)}:${global.init.formatIntZero(nowTime.Minute, -2)}`;
  mp.gui.execute(`HUD.time='${time}'`);
  global.oldchat.execute(`chat.stime = '${time}'`);
});
...
mp.events.add('Enviroment_Date', (data) => {
  ...
  const date = `${global.init.formatIntZero(nowDate.Day, -2)}.${global.init.formatIntZero(nowDate.Month, -2)}.${nowDate.Year}`;
...
//mp.console.logInfo('file');
mp.events.add('Enviroment_Start', (timeData, dateData, weather) => {
 // mp.console.logInfo('check');
  mp.game.time.setClockTime(timeData[0], timeData[1], 0);
  //mp.console.logInfo(`EnvStart: ClockTime: ${timeData[0]}:${timeData[1]}:0 Weather: ${weather}`);
