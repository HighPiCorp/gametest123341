...
namespace NeptuneEvo.Core
{
    class EventSys : Script
    {
        ...
        private static Dictionary<Player, playerData> playersDataBeforeEnterEvent = new Dictionary<Player, playerData>();
        ...
        [Command("createmp", "Используйте: /createmp [Лимит участников] [Радиус зоны] [Название мероприятия]", GreedyArg = true, Hide = true)]
            ...
            if (Main.Players.ContainsKey(player))
            {
                ...
                if (AdminEvent.EventState == 0)
                {
                    if (eventname.Length < 50)
                    {
                        ...
                        AdminEvent.Zone = NAPI.ColShape.CreateSphereColShape(player.Position, radius, player.Dimension);
                        ...
                        AddAdminEventLog();
                        NAPI.Chat.SendChatMessageToAll("!{#d9dc12}Уважаемые игроки, скоро начнётся мероприятие '" + eventname + "'!");
                        if (members != 0) NAPI.Chat.SendChatMessageToAll("!{#d9dc12}На данном мероприятии установлен лимит участников: " + members + ".");
                        else NAPI.Chat.SendChatMessageToAll("!{#d9dc12}На данном мероприятии не установлен лимит участников.");
                        if (AdminEvent.Zone != null) NAPI.Chat.SendChatMessageToAll("!{#d9dc12}Мероприятие действует в зоне " + radius + "м от точки телепорта.");
                        NAPI.Chat.SendChatMessageToAll("!{#d9dc12}Чтобы телепортироваться на мероприятие, введите команду /mp");
        ...
        public void StartEvent(Player player)
        {
            if (Main.Players.ContainsKey(player))
            {
                ...
                if (AdminEvent.EventState == 1)
                {
                    ...
                    {
                        playersDataBeforeEnterEvent = new Dictionary<Player, playerData>();
                        AdminEvent.EventState = 2;
                        NAPI.Chat.SendChatMessageToAll("!{#d9dc12}Мероприятие '" + AdminEvent.Name + "' началось, телепорт закрыт!");
                        NAPI.Chat.SendChatMessageToAll("!{#d9dc12}Игроков на мероприятии: " + AdminEvent.EventMembers.Count + ".");
                    }
        ...
        [Command("stopmp", "Используйте: /stopmp [ID игрока] [Награда]", Hide = true)]
        ...
        [Command("mpveh", "Используйте: /mpveh [Название модели] [Цвет] [Цвет] [Количество машин]", Hide = true)]
        ...
        [Command("mpreward", "Используйте: /mpreward [Новый лимит]", Hide = true)]
        ...
        public void TpToMp(Player player)
        {
            if (Main.Players.ContainsKey(player))
            {
                if (Main.Players[player].DemorganTime == 0 && Main.Players[player].ArrestTime == 0 && player.HasMyData("CUFFED") && player.GetMyData<bool>("CUFFED") == false && player.HasSharedData("InDeath") && player.GetSharedData<bool>("InDeath") == false)
                {
                    if (AdminEvent.EventState == 1)
                    {
                        if (!AdminEvent.EventMembers.Contains(player))
                        {
                            ...
                            {
                                playerData data = new playerData(player.Position, player.Health, player.Dimension);
                                playersDataBeforeEnterEvent.Add(player,data);
        ...
        [Command("mpkick", "Используйте: /mpkick [ID игрока]", Hide = true)]
        ...
        [Command("mphp", "Используйте: /mphp [Количество HP]", Hide = true)]
        ...
        [Command("mpar", "Используйте: /mpar [Количество Armor]", Hide = true)]
        ...
        private void CloseAdminEvent(Player winner, uint reward)
        {
            ...
            if (Main.Players.Keys.Contains(winner))
            {
                Wallet.Change(winner, (int)reward);
            }
            AdminEvent.EventState = 0;
            UpdateLastAdminEventLog();
            NAPI.Chat.SendChatMessageToAll("!{#d9dc12}Мероприятие '" + AdminEvent.Name + "' закончилось, спасибо за участие!");
            if (winner != AdminEvent.Admin)
            {
                if (reward != 0)
                {
                    NAPI.Chat.SendChatMessageToAll("!{#d9dc12}Победитель " + winner.Name + " получил приз в размере " + reward + "$.");
                    MoneySystem.Wallet.Change(winner, (int)reward);

                }
                else NAPI.Chat.SendChatMessageToAll("!{#d9dc12}Победитель: " + winner.Name + ".");
                client.Core.Achievements.AddAchievementScore(winner, client.Core.AchievementID.WinEvents, 1);
            }
        }

        private void RemovePlayerFromEvent(Player player)
        {
            if (!playersDataBeforeEnterEvent.ContainsKey(player) || !Main.Players.ContainsKey(player)) return;
            playerData data = new playerData(playersDataBeforeEnterEvent[player]);
            player.Position = data.playerPosition;
            player.Health = data.playerHp;
            player.Dimension = data.dimension;
            playersDataBeforeEnterEvent.Remove(player);
        }
        ...
        private class playerData
        {
            ...
            public playerData(Vector3 pos, int hp, uint dim)
            {
                playerPosition = pos;
                playerHp = hp;
                dimension = dim;
