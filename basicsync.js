...
mp.events.add('attachObjectToVehicle', attachObjectToVehicle);
...
function attachObjectToVehicle(player) {
    try {
        if (player && mp.players.exists(player)) {
            if (attachedObjects[player.id] != undefined) attachedObjects[player.id].destroy();
            ...
            if (player.getVariable('attachedObject') == null) return;
            let data = JSON.parse(player.getVariable('attachedObject'));
            let boneID = player.vehicle.getBoneIndexByName(data.BoneName);
            var object = mp.objects.new(data.Model, player.position,
                {
                    rotation: new mp.Vector3(0, 0, 0),
                    alpha: 255,
            ...
            waitEntity(object).then(() => {
                object.attachTo(player.vehicle.handle, boneID, data.PosOffset.x, data.PosOffset.y, data.PosOffset.z, data.RotOffset.x, data.RotOffset.y, data.RotOffset.z, true, true, false, false, 0, true);
                attachedObjects[player.id] = object;
               // object.setAlpha(255);
            });
        }
...
mp.game.streaming.requestAnimDict("amb@prop_human_seat_chair@female@proper@base");

mp.events.add('entityStreamIn', function (entity) {
    try {
        if (entity.type === 'player') {
            SetWalkStyle(entity, global.walkstyles[entity.getVariable('playerws')]);
            SetMood(entity, global.moods[entity.getVariable('playermood')]);
            attachObject(entity);
            ...
            else if(entity.getModel() == 826475330) entity.taskPlayAnim("amb@prop_human_seat_chair@female@proper@base", "base", 8.0, 1.0, -1, 1, 0.0, false, false, false); // Debra TEST
        }

    } catch (e) { }
});

mp.events.add('client_taskPlayAnim', function (player, animLib, animName) {
    try {
        if (mp.players.exists(player)) {
            loadAnimDict(animLib, () => {
            player.taskPlayAnim(animLib, animName, 1, 0, 6e3, 49, 1, !1, !1, !1);
            });
...
mp.events.add('client_stopTask', function (player, animLib, animName) {
    try {
        if (mp.players.exists(player)) {
            player.stopAnimTask(animLib, animName, 3.0);
        }
    } catch (e) { }
});
